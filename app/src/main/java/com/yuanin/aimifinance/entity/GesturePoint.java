...
public class GesturePoint {
    ...
    public void setPointState(Context context, int state) {
        pointState = state;
        switch (state) {
            case StaticMembers.POINT_STATE_NORMAL:
                //此方法更节省内存 防止OOM
                this.image.setBackgroundDrawable(new BitmapDrawable(context.getResources(), AppUtils.getBitmap(context, R.mipmap.gesture_node_normal)));
//                this.image.setImageResource(R.mipmap.gesture_node_normal);
                break;
            case StaticMembers.POINT_STATE_SELECTED:
                //此方法更节省内存 防止OOM
                this.image.setBackgroundDrawable(new BitmapDrawable(context.getResources(), AppUtils.getBitmap(context, R.mipmap.gesture_node_pressed)));
//                this.image.setImageResource(R.mipmap.gesture_node_pressed);
                break;
            case StaticMembers.POINT_STATE_WRONG:
                //此方法更节省内存 防止OOM
                this.image.setBackgroundDrawable(new BitmapDrawable(context.getResources(), AppUtils.getBitmap(context, R.mipmap.gesture_node_pressed)));
//                this.image.setImageResource(R.mipmap.gesture_node_pressed);
                break;
