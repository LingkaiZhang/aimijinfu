...
import com.yuanin.aimifinance.entity.GesturePoint;
...
public class GestureDrawline extends View {
    ...
    public GestureDrawline(Context context, List<GesturePoint> list,
        ...
        super(context);
        screenDispaly = AppUtils.getScreenDispaly(context);
        paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
        bitmap = Bitmap.createBitmap(screenDispaly[0],
                screenDispaly[0], Bitmap.Config.ARGB_4444); // 设置位图的宽高
        canvas = new Canvas();
        canvas.setBitmap(bitmap);
        paint.setStyle(Style.STROKE);// 设置非填充
        paint.setColor(context.getResources().getColor(R.color.theme_color));// 设置默认连线颜色
        paint.setStrokeWidth(6);// 笔宽12像素
//        paint.setAlpha(12000);
        paint.setAntiAlias(false);// 不显示锯齿
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (currentPoint != null) {
                    currentPoint.setPointState(context, StaticMembers.POINT_STATE_SELECTED);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (pointAt == null
                        ...
                        || StaticMembers.POINT_STATE_SELECTED == pointAt
                        .getPointState()) {
                    ...
                    pointAt.setPointState(context, StaticMembers.POINT_STATE_SELECTED);

                    // 判断是否中间点需要选中
                    GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint,
                            pointAt);
                    if (betweenPoint != null
                            && StaticMembers.POINT_STATE_SELECTED != betweenPoint
                            .getPointState()) {
                        // 存在中间点并且没有被选中
                        ...
                        // 设置中间点选中
                        betweenPoint.setPointState(context, StaticMembers.POINT_STATE_SELECTED);
                        // 赋值当前的point;
                        currentPoint = pointAt;
                    } else {
    ...
    final class clearStateRunnable implements Runnable {
        public void run() {
            ...
            for (GesturePoint p : list) {
                p.setPointState(context, StaticMembers.POINT_STATE_NORMAL);
            }
            invalidate();
            isDrawEnable = true;
        }
    ...
    private void drawErrorPathTip() {
        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        // paint.setStrokeWidth(27);// 笔宽5像素
        // paint.setAlpha(100);
        // paint.setColor(Color.rgb(255, 255, 255));// 设置默认线路颜色
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
            pair.first.setPointState(context, StaticMembers.POINT_STATE_WRONG);
            pair.second.setPointState(context, StaticMembers.POINT_STATE_WRONG);
            canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
                    pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
        }
        invalidate();
