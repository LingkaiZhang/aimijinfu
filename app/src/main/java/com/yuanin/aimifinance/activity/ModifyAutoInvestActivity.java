 ...
 * 修改自动投资
 */
public class ModifyAutoInvestActivity extends BaseActivity {
    ...
    private AutoInvestEntity data;
    private Context context = ModifyAutoInvestActivity.this;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_modify_auto_invest);
        x.view().inject(this);
        initTopBar("修改自动投资", toptitleView, true);
        data = (AutoInvestEntity) getIntent().getSerializableExtra("data");
        initData();
        //讲光标放在文末
    ...
    private void initData() {
        ...
        if (data.getMin_unit().equals(data.getMax_unit())) {
            if (data.getMin_unit().equals("0")) {
                ...
                int day = Integer.parseInt(data.getMin_unit()) * 30;
                tvTime.setText(data.getMin_unit() + "个月  (" + day + "天)");
                minDate = Integer.parseInt(data.getMin_unit());
                maxDate = Integer.parseInt(data.getMin_unit());
            }
        } else {
            int day1 = Integer.parseInt(data.getMin_unit()) * 30;
            int day2 = Integer.parseInt(data.getMax_unit()) * 30;
            tvTime.setText(data.getMin_unit() + "~" + data.getMax_unit() + "个月  (" + day1 + "~" + day2 + "天)");
            minDate = Integer.parseInt(data.getMin_unit());
            maxDate = Integer.parseInt(data.getMax_unit());
        }
        ...
        if (data.getMin_annual().equals(data.getMax_annual())) {
            if (data.getMin_annual().equals("0")) {
                ...
                tvRate.setText(data.getMin_annual() + "%");
                minRate = Integer.parseInt(data.getMin_annual());
                maxRate = Integer.parseInt(data.getMin_annual());
            }
        } else {
            tvRate.setText(data.getMin_annual() + "%~" + data.getMax_annual() + "%");
            minRate = Integer.parseInt(data.getMin_annual());
            maxRate = Integer.parseInt(data.getMax_annual());
        }

        if (data.getAll_amount().equals("1")) {
            investType = 1;
            ...
            etMoney.setText(data.getAmount());
        }

        if (data.getIs_red().equals("1")) {
            toggleUse.setChecked(true);
            isUseRed = 1;
        } else {
            toggleUse.setChecked(false);
            isUseRed = 0;
        }
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.btnConfirm:
                if (etName.getText().toString().trim().length() == 0) {
                    AppUtils.showToast(context, "请输入方案名称");
                    etName.requestFocus();
                ...
                if (minRate == 0 && maxRate == 0 && minDate == 0 && maxDate == 0) {
                    AppUtils.showToast(context, "请至少选择一个利率区间或者期限区间");
                    return;
                }
                ...
                if (investType == 0) {
                    if (etMoney.getText().toString().trim().length() > 0) {
                        ...
                        if (money < 1000) {
                            AppUtils.showToast(context, "请输入大于1000的金额");
                            etMoney.requestFocus();
                            AppUtils.openKeyboard(etMoney);
                            return;
                        }
                    } else {
                        AppUtils.showToast(context, "请输入自动投资金额");
                        etMoney.requestFocus();
                        AppUtils.openKeyboard(etMoney);
                        return;
                    }
                ...
                NetUtils.request(context, obj, mType, new IHttpRequestCallBack() {
                            ...
                            public void onSuccess(Object object) {
                                ...
                                if (entity.isSuccess(context)) {
                                    ModifyAutoInvestActivity.this.finish();
                                }
